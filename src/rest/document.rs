...
async fn get_document(
    ...
    if document.paste_id != paste_id {
        return Err(AppError::BadRequest("The document ID does not belong to that paste.".to_string()));
    }
...
async fn post_document(
    ...
    if (app.config.global_paste_total_document_size_limit() * 1024 * 1024) < (total_document_size + body.len()) {
        return Err(AppError::BadRequest(
